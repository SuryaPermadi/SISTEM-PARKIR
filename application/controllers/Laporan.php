...
class Laporan extends CI_Controller {
	...
	public function laportanggal($value=''){
		...
		// Inisialisasi $total sebagai array kosong
		$total = array();
		...
		// Periksa apakah ada data dalam $total sebelum melakukan array_sum()
		if (!empty($total)) {
			$data['total'] = array_sum($total);
		} else {
			$data['total'] = 0; // Set nilai total menjadi 0 jika tidak ada data
		}
