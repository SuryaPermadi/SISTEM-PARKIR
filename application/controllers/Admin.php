...
class Admin extends CI_Controller
{
	...
	public function view($id = '')
	{
		...
		if ($sqlcek) {
			if ($sqlcek['level_admin'] == '1') {
				$data['hak_akses'] = 'Owner';
			} else if ($sqlcek['level_admin'] == '2') {
				$data['hak_akses'] = 'Petugas';
			}
			$data['title'] = 'View Admin';
			$data['admin'] = $sqlcek;
			// die(print_r($data));
			$this->load->view('viewadmin', $data, FALSE);
		} else {
			$this->session->set_flashdata('message', 'swal("Gagal", "Data Admin Tidak Ada", "error");');
			redirect('admin');
		}
	}
	function update()
